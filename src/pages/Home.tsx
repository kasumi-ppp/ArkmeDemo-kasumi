...
import ArrangementDraftModal from "@/components/ArrangementDraftModal";
    ...
    if (settingsView === "settings") {
      return (
        <SettingsScreen
          ...
          onOpenAiSettings={() => onNavigate("ai-settings")}
        />
      );
    }
...
function SendToSelfConversationChat({
  ...
  const [currentDraft, setCurrentDraft] = React.useState<Partial<ArrangementItem> | null>(null);
  ...
  const handleSubmit = async (content: string) => {
    onCreateRecord(content);

    if (preferences.aiEnabled) {
      try {
        const { createDraftArrangement } = await import("@/services/aiParser");
        const draft = await createDraftArrangement(content);
        if (draft) {
  ...
  const handleConfirmDraft = () => {
    ...
    const { createArrangement } = require("@/data/arrangements");
    const arrangement = createArrangement(
      currentDraft.title || "",
      currentDraft.note || "",
      currentDraft.scheduledAt || null,
      currentDraft.people || [],
      currentDraft.location || "",
      currentDraft.priority || "medium",
      currentDraft.contexts || [],
      currentDraft.source || "ai",
      false,
      currentDraft.aiConfidence || 0,
      currentDraft.origin || null
    );
    const { persistArrangements, getInitialArrangements } = require("@/data/arrangements");
    const existing = getInitialArrangements();
    persistArrangements([...existing, arrangement]);
  ...
  return (
    <div className="flex h-full flex-col bg-bg">
      ...
      <ChatInput
        ...
        onVoiceSubmit={() => handleSubmit(t("records.voiceRecord"))}
      />

      {showDraftModal && currentDraft && (
        <ArrangementDraftModal
          draft={currentDraft}
          onConfirm={handleConfirmDraft}
          onEdit={handleEditDraft}
          onIgnore={handleIgnoreDraft}
          onClose={handleIgnoreDraft}
        />
      )}
    </div>
  );
...
function TestIdentityConversationChat({
  ...
  const { resolvedLocale, t, preferences } = usePreferences();
  ...
  const handleSubmit = async (content: string) => {
    ...
    if (preferences.aiEnabled && summary.conversationType === "private") {
      try {
        const { parseChatForArrangement } = await import("@/services/aiParser");
        
        const messages = sortedRecords.map(r => ({
          content: r.text_content,
          senderId: r.sender === "demo" ? "self" : r.identityId,
          senderName: r.sender === "demo" 
            ? selfDisplayName 
            : summary.memberIdentities.find(i => i.id === r.identityId)?.name
        }));
        
        messages.push({ content, senderId: "self", senderName: selfDisplayName });
        
        const draft = await parseChatForArrangement(messages, "self");
        if (draft) {
          setCurrentDraft(draft);
          setShowDraftModal(true);
        }
      } catch (e) {
        console.error("AI chat parsing failed:", e);
      }
    }
  };
